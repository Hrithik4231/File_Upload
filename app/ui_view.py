...
#         self.display_file_cards(metadata)

#         # Preview PDF below all cards if one is selected
#         if st.session_state.selected_preview_file_id:
#             selected_id = st.session_state.selected_preview_file_id
#             if selected_id in metadata:
#                 self.render_pdf_preview(selected_id, metadata[selected_id])

#     def display_file_cards(self, metadata: Dict):
#         files_per_row = 3
...
#                 if i + j < len(file_items):
#                     file_id, file_info = file_items[i + j]
#                     self.create_file_card(col, file_id, file_info)

#     def create_file_card(self, col, file_id: str, file_info: Dict):
#         with col:
#             with st.container():
#                 st.markdown(
#                     """
...
#                 st.write(f"**Size:** {self.file_manager.format_file_size(file_info['filesize'])}")
#                 st.write(f"**Created:** {self.file_manager.format_datetime(file_info['created_at'])}")
...
#                 with col1:
#                     if st.button(f"👁️ Preview", key=f"preview_{file_id}"):
...
#                 st.markdown(f"### 📄 Preview: {file_info['filename']}", unsafe_allow_html=True)

#                 pdf_display = f"""
#                 <div style="display: flex; justify-content: center;">
#                     <iframe src="data:application/pdf;base64,{base64_pdf}" 
#                             width="1100" height="500" type="application/pdf" style="border: 2px solid #555; border-radius: 8px;">
#                     </iframe>
#                 </div>
#                 """
